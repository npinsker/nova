...
class MapEditor extends FlxLocalSprite implements Focusable {
  ...
  public function refresh() {
    ...
    if (texts[selectedIndex].y < 50) {
        var adj = 50 - texts[selectedIndex].y;
        for (i in 0...texts.length) {
            texts[i].y += adj;
        }
    }
    if (texts[selectedIndex].y > FlxG.height - 100) {
        var adj = texts[selectedIndex].y - (FlxG.height - 100);
        for (i in 0...texts.length) {
            texts[i].y -= adj;
        }
    }
  ...
  public function handleInput():Void {
    if (mode == SELECTING) {
      ...
      if (InputController.justPressed(LEFT)) {
        selectedIndex -= 10;
        if (selectedIndex < 0) selectedIndex = 0;
        refresh();
      }
      if (InputController.justPressed(RIGHT)) {
        selectedIndex += 10;
        if (selectedIndex >= texts.length) selectedIndex = texts.length - 1;
        refresh();
      }

  ...
  /**
    ...
    */
	public static function fromGraphic(graphic:FlxGraphicAsset, ?options:LoadGraphicOptions):LocalSpriteWrapper {
		...
		if (options.frameSize == null && options.animation == null) {
			return new LocalSpriteWrapper(new FlxSprite().loadGraphic(bitmap));
		}
		...
		if (options.animation != null) {
			...
			wrapper._sprite.animation.add('default', options.animation, (options.frameRate != null ? options.frameRate : 30));
			wrapper._sprite.animation.play('default');
			
			return wrapper;
		}
		
		var wrapper = new LocalSpriteWrapper(new FlxSprite().loadGraphic(bitmap, true, options.frameSize.x, options.frameSize.y));
		return wrapper;
	}
}
